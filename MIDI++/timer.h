...
/**
 * Calibrate TSC frequency by running multiple passes IN PARALLEL and picking the median.
 */
	...
	double measureTimeSec = MEASURE_SEC)
{
	...
	// Launch all calibration passes concurrently
	std::vector<std::future<double>> futures;
	futures.reserve(passes);

	for (int i = 0; i < passes; ++i) {
		futures.push_back(std::async(std::launch::async, [measureTimeSec]() {
			return __measure_tsc_freq_once(measureTimeSec, 0 /* pin to core 0 */);
			}));
	}
