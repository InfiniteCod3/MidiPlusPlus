...
void VirtualPianoPlayer::prepare_event_queue() {
	...
	if (midi::Config::getInstance().legit.ENABLED) {
		auto& l = midi::Config::getInstance().legit;
		static std::mt19937 gen{ std::random_device{}() };
		std::uniform_real_distribution<double> prob(0.0, 1.0);
		std::uniform_real_distribution<double> jitter(-l.TIMING_VARIATION, l.TIMING_VARIATION);
		std::uniform_real_distribution<double> extra(l.EXTRA_DELAY_MIN, l.EXTRA_DELAY_MAX);

		std::vector<NoteEvent*> transformed;
		transformed.reserve(note_buffer.size());
		for (auto* e : note_buffer) {
			if (!e->isSustain) {
				if (e->action == EventType::Press) {
					double r = prob(gen);
					if (r < l.NOTE_SKIP_CHANCE) {
						continue;
					}
					double j = jitter(gen);
					double d = (prob(gen) < l.EXTRA_DELAY_CHANCE) ? extra(gen) : 0.0;
					auto delta = std::chrono::nanoseconds(static_cast<long long>((j + d) * 1e9));
					auto t = e->time + delta;
					if (t < std::chrono::nanoseconds(0)) t = std::chrono::nanoseconds(0);
					e->time = t;
				}
			}
			transformed.push_back(e);
		}
		note_buffer.swap(transformed);
		std::stable_sort(note_buffer.begin(), note_buffer.end(), [](const NoteEvent* a, const NoteEvent* b) { return a->time < b->time; });
		std::cout << "[LEGIT] Enabled | delayChance=" << l.EXTRA_DELAY_CHANCE << " min=" << l.EXTRA_DELAY_MIN << " max=" << l.EXTRA_DELAY_MAX
			<< " skipChance=" << l.NOTE_SKIP_CHANCE << " jitter=" << l.TIMING_VARIATION << "\n";
	}
}
...
void VirtualPianoPlayer::initializeKeyCache() {
	// Collect all unique keys that need computation
	std::vector<std::string> keysToCompute;

	for (const auto& [note, key] : limited_key_mappings) {
		if (!key.empty() && g_keyCache.find(key) == g_keyCache.end()) {
			keysToCompute.push_back(key);
		}
	}
	for (const auto& [note, key] : full_key_mappings) {
		if (!key.empty() && g_keyCache.find(key) == g_keyCache.end()) {
			// Avoid duplicates
			if (std::find(keysToCompute.begin(), keysToCompute.end(), key) == keysToCompute.end()) {
				keysToCompute.push_back(key);
			}
		}
	}

	if (keysToCompute.empty()) {
		return; // Nothing to compute
	}

	// Parallelize computation using the thread pool
	std::mutex cacheMutex;
	std::vector<std::future<void>> futures;
	futures.reserve(keysToCompute.size());

	for (const auto& key : keysToCompute) {
		futures.push_back(processing_pool.enqueue([&key, &cacheMutex]() {
			// Compute the key sequence (CPU-intensive work)
			KeySequence seq = computeKeySequence(key);

			// Thread-safe insertion into the cache
			std::lock_guard<std::mutex> lock(cacheMutex);
			g_keyCache.emplace(key, std::move(seq));
			}));
	}
