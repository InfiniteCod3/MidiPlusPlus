...
namespace midi {
	...
	void LegitModeSettings::validate() const {
		if (EXTRA_DELAY_CHANCE < 0.0 || EXTRA_DELAY_CHANCE > 1.0)
			throw ConfigException("EXTRA_DELAY_CHANCE must be between 0 and 1");
		if (NOTE_SKIP_CHANCE < 0.0 || NOTE_SKIP_CHANCE > 1.0)
			throw ConfigException("NOTE_SKIP_CHANCE must be between 0 and 1");
		if (EXTRA_DELAY_MIN < 0.0 || EXTRA_DELAY_MAX < 0.0)
			throw ConfigException("EXTRA_DELAY_MIN/MAX cannot be negative");
		if (EXTRA_DELAY_MIN > EXTRA_DELAY_MAX)
			throw ConfigException("EXTRA_DELAY_MIN cannot be greater than EXTRA_DELAY_MAX");
		if (TIMING_VARIATION < 0.0)
			throw ConfigException("TIMING_VARIATION cannot be negative");
	}
	...
	void from_json(const json& j, Config& c) {
		...
		if (j.contains("LEGIT_MODE_SETTINGS")) {
			j.at("LEGIT_MODE_SETTINGS").get_to(c.legit);
		}
		else {
			c.legit = { false, 0.05, 0.05, 0.2, 0.02, 0.1 };
		}
	}
