...
// MIDI scanning synchronization
static std::mutex        g_midiScanMutex;
static std::atomic<bool> g_midiScanInProgress{ false };
...
namespace Layout {
	...
	static const int DET_Y = LEGIT_Y + LEGIT_H + 5;
...
enum ControlID {
	...
	ID_GRP_LEGIT = 4000,
	ID_CHK_LEGIT_ENABLE,
	ID_EDIT_LEGIT_DELAY_CHANCE,
	ID_EDIT_LEGIT_DELAY_MIN,
	ID_EDIT_LEGIT_DELAY_MAX,
	ID_EDIT_LEGIT_SKIP_CHANCE,
	ID_EDIT_LEGIT_TIMING_VAR,
	ID_BTN_LEGIT_INFO
};
...
// Async version that runs in background and notifies UI when complete
static void ScanMidiFolderAsync(HWND hwnd) {
	if (g_midiScanInProgress.exchange(true)) {
		return; // Scan already in progress
	}

	std::thread([hwnd]() {
		{
			std::lock_guard<std::mutex> lock(g_midiScanMutex);
			ScanMidiFolder();
		}
		g_midiScanInProgress.store(false);
		PostMessage(hwnd, WM_MIDI_SCAN_COMPLETE, 0, 0);
		}).detach();
}
...
static void UpdateMidiDetails() {
	...
	auto& l = midi::Config::getInstance().legit;
	std::ostringstream lss;
	lss << "\r\nLegit Mode: " << (l.ENABLED ? "On" : "Off");
	if (l.ENABLED) {
		lss << " | delayChance=" << std::fixed << std::setprecision(2) << l.EXTRA_DELAY_CHANCE
			<< " min=" << std::setprecision(2) << l.EXTRA_DELAY_MIN
			<< " max=" << std::setprecision(2) << l.EXTRA_DELAY_MAX
			<< " skipChance=" << std::setprecision(2) << l.NOTE_SKIP_CHANCE
			<< " jitter=" << std::setprecision(2) << l.TIMING_VARIATION;
	}
	SendMessageA(g_editDetails, EM_REPLACESEL, FALSE, reinterpret_cast<LPARAM>(lss.str().c_str()));
}
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CREATE:
	{
		...
		CreateWindowW(L"button", L"Legit Mode",
			WS_CHILD | WS_VISIBLE | BS_GROUPBOX,
			Layout::LEGIT_X, Layout::LEGIT_Y, Layout::LEGIT_W, Layout::LEGIT_H,
...

		{
			auto& l = midi::Config::getInstance().legit;
			wchar_t buf[64];
			swprintf_s(buf, L"%.3f", l.EXTRA_DELAY_CHANCE);
			SetWindowTextW(hEditDelayChance, buf);
			swprintf_s(buf, L"%.3f", l.EXTRA_DELAY_MIN);
			SetWindowTextW(hEditDelayMin, buf);
			swprintf_s(buf, L"%.3f", l.EXTRA_DELAY_MAX);
			SetWindowTextW(hEditDelayMax, buf);
			swprintf_s(buf, L"%.3f", l.NOTE_SKIP_CHANCE);
			SetWindowTextW(hEditSkipChance, buf);
			swprintf_s(buf, L"%.3f", l.TIMING_VARIATION);
			SetWindowTextW(hEditTimingVar, buf);
		}
		...
		// Initial Setup - Start async MIDI scanning
		SendMessageW(g_lbMidi, LB_ADDSTRING, 0, reinterpret_cast<LPARAM>(L"Loading MIDI files..."));
		ScanMidiFolderAsync(hWnd);
	...
	case WM_COMMAND:
	{
		...
		case ID_CHK_LEGIT_ENABLE:
			if (code == BN_CLICKED) {
				HWND hChk = reinterpret_cast<HWND>(lParam);
				bool en = (SendMessage(hChk, BM_GETCHECK, 0, 0) == BST_CHECKED);
				midi::Config::getInstance().legit.ENABLED = en;
				try {
					midi::Config::getInstance().saveToFile("config.json");
				}
				catch (const std::exception& ex) {
					std::cerr << "[Config] Failed to save legit: " << ex.what() << "\n";
				}
				if (en) {
					MessageBoxA(hWnd,
						"Legit Mode enabled. It adds occasional delays, timing variation, and optional note skips to emulate human playing.\n\nChances are fractions in [0,1]. Delays and variation are in seconds.",
						"Legit Mode",
		...
		case ID_EDIT_LEGIT_DELAY_CHANCE:
		case ID_EDIT_LEGIT_DELAY_MIN:
		case ID_EDIT_LEGIT_DELAY_MAX:
		case ID_EDIT_LEGIT_SKIP_CHANCE:
		case ID_EDIT_LEGIT_TIMING_VAR:
			if (code == EN_KILLFOCUS || code == EN_CHANGE) {
				auto readDouble = [&](int ctrlId) -> double {
					wchar_t t[64];
					GetWindowTextW(GetDlgItem(hWnd, ctrlId), t, 64);
					wchar_t* endp = nullptr;
					double v = wcstod(t, &endp);
					return v;
					};
				auto& l = midi::Config::getInstance().legit;
				double dc = readDouble(ID_EDIT_LEGIT_DELAY_CHANCE);
				double dmin = readDouble(ID_EDIT_LEGIT_DELAY_MIN);
				double dmax = readDouble(ID_EDIT_LEGIT_DELAY_MAX);
				double sc = readDouble(ID_EDIT_LEGIT_SKIP_CHANCE);
				double tv = readDouble(ID_EDIT_LEGIT_TIMING_VAR);
				dc = std::clamp(dc, 0.0, 1.0);
				sc = std::clamp(sc, 0.0, 1.0);
				if (dmin < 0.0) dmin = 0.0;
				if (dmax < 0.0) dmax = 0.0;
				if (dmin > dmax) dmin = dmax;
				if (tv < 0.0) tv = 0.0;
				l.EXTRA_DELAY_CHANCE = dc;
				l.EXTRA_DELAY_MIN = dmin;
				l.EXTRA_DELAY_MAX = dmax;
				l.NOTE_SKIP_CHANCE = sc;
				l.TIMING_VARIATION = tv;
				try {
					midi::Config::getInstance().saveToFile("config.json");
				}
				catch (const std::exception& ex) {
					std::cerr << "[Config] Failed to save legit params: " << ex.what() << "\n";
				}
			}
			break;
	...
	case WM_MIDI_SCAN_COMPLETE:
	{
		// Background MIDI scan is complete, update the UI
		std::lock_guard<std::mutex> lock(g_midiScanMutex);
		SortMidiItems();
		PopulateMidiList();
		UpdateWindowFocusability();
